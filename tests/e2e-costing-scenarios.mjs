  ...
  try {
    const partDefs = [
      ...
      },
      {
        ...
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'pcb_rigid',
        material_grade:        'FR4 Tg150, 2-layer, 1.6mm, HASL lead-free, copper weight 1oz/35µm both layers',
        net_weight_g:          20,
        dimensions_json:       { l_mm: 60, w_mm: 40, h_mm: 1.6, layers: 2, smd_components: 18, tht_components: 3, solder_pads: 95, vias: 12 },
        surface_finish:        'HASL lead-free, green soldermask both sides, white silkscreen top, IPC Class 2',
        tolerance_class:       'Outer dimension ±0.2mm, track/gap 0.15/0.15mm, via drill Ø0.3mm ±0.05mm, annular ring ≥0.1mm',
        manufacturing_process: '2-layer PCB fab (drill → PTH copper plating → imaging → etch → soldermask → HASL) → SMT pick-and-place 18 components → reflow soldering → THT insertion 3 components → wave solder → AOI 100% inspection',
      },
      {
        part_name:             'AL6061-T6 Housing End Cap CNC Ø50mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'cnc_machining',
        material_grade:        'Aluminium 6061-T6, 2.70g/cm³, stock: round bar Ø55mm × L20mm',
        net_weight_g:          72,
        dimensions_json:       { diameter_mm: 50, h_mm: 12, bore_mm: 44, wall_mm: 3, bore_type: 'through-bore', thread: 'M50x1_external_6g' },
        surface_finish:        'Type II anodised clear 10µm, Ra 0.8µm sealing face, Ra 1.6µm OD, chamfer 1×45° both ends',
        tolerance_class:       'M50×1 thread 6g IT6, sealing face flatness 0.01mm, through-bore Ø44mm H8, OD Ø50mm h9',
        manufacturing_process: 'CNC turning: face + OD Ø50mm + through-bore Ø44mm H8 → threading M50×1 external → sealing face finish Ra 0.8µm → chamfering → Type II anodise clear',
      },
    ]
