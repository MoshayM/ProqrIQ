...
async function scenario10_BulkCosting() {
  ...
  try {
    const partDefs = [
      ...
      {
        part_name:             'PA12 Cable Gland Body M20×1.5 — IP68 injection moulded',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'plastic_injection',
        material_grade:        'Polyamide PA12 natural, 1.01g/cm³, UL94-HB, rated −40°C to +100°C',
        net_weight_g:          18,
        dimensions_json:       { l_mm: 38, od_mm: 28, thread: 'M20x1.5', wall_mm: 2.5 },
        surface_finish:        'As-moulded, Ra 1.6µm, no secondary finish; IP68 seal via O-ring groove',
        tolerance_class:       'M20×1.5 thread 6H, OD ±0.3mm, wall ±0.2mm, ISO 286 general IT12',
        manufacturing_process: 'Injection moulding PA12 (1-cavity tool, hot runner) → automatic degating → visual inspect 5% AQL 1.0',
      },
      {
        part_name:             'AL6061-T6 Housing End Cap CNC Ø50mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'cnc_machining',
        material_grade:        'AL6061-T6, stock round bar Ø55mm × L=12mm finished',
        net_weight_g:          72,
        dimensions_json:       { diameter_mm: 50, l_mm: 12, bore_mm: 44, wall_mm: 3, bore_type: 'through', thread: 'M50x1_6g', sealing_face: 'one_end' },
        surface_finish:        'Type II anodise clear subcontracted; Ra 0.8µm sealing face one end; sample inspection 10%',
        tolerance_class:       'M50×1 thread 6g, bore Ø44 H8, OD Ø50 h9, face flatness 0.01mm',
        manufacturing_process: 'CNC turn OD + through-bore Ø44 H8 + M50×1 thread → face Ra 0.8µm → chamfer → subcontract anodise',
      },
    ]
