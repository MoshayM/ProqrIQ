  ...
  try {
    const partDefs = [
      ...
      },
      {
        ...
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'pcb_rigid',
        material_grade:        'FR4 Tg150, 2-layer, 1.6mm, HASL lead-free, copper weight 1oz/35µm both layers; BOM: 12× 0402 resistor/cap, 4× PLCC-2 LED, 2× SOT-23 transistor, 1× 2×5 pin connector THT',
        net_weight_g:          20,
        dimensions_json:       { l_mm: 60, w_mm: 40, thickness_mm: 1.6, layers: 2, smd_components: 18, tht_components: 3, solder_pads: 95, vias: 12 },
        surface_finish:        'HASL lead-free, green soldermask both sides, white silkscreen top, IPC Class 2; AOI only — no X-ray, no functional test; loaded labour rate €48/hr',
        tolerance_class:       'Outer dimension ±0.2mm, track/gap 0.15/0.15mm, via drill Ø0.3mm ±0.05mm, annular ring ≥0.1mm',
        manufacturing_process: 'PCB fab at CN supplier (Asian) → SMT: Yamaha YSM20R pick-and-place (18 SMD) → Heller 1707 MKIII reflow → THT insertion 3 components → selective wave solder → AOI 100% → ship to DE; no in-house fab',
      },
      {
        part_name:             'AL6061-T6 Housing End Cap CNC Ø50mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'cnc_machining',
        material_grade:        'Aluminium 6061-T6, 2.70g/cm³, stock: round bar Ø55mm × L20mm (L=12mm finished)',
        net_weight_g:          72,
        dimensions_json:       { diameter_mm: 50, l_mm: 12, bore_mm: 44, wall_mm: 3, bore_type: 'through-bore', thread: 'M50x1_external_6g', sealing_face: 'one_end_only' },
        surface_finish:        'Type II anodised clear 10µm subcontracted; Ra 0.8µm sealing face (one end only); Ra 1.6µm OD; chamfer 1×45° both ends; sampling inspection 10% + thread gauge every batch',
        tolerance_class:       'M50×1 thread 6g IT6, sealing face flatness 0.01mm, through-bore Ø44mm H8, OD Ø50mm h9; CMM sampling 10%; no 100% inspection',
        manufacturing_process: 'CNC turning: face + OD Ø50mm + through-bore Ø44mm H8 → threading M50×1 external → sealing face finish Ra 0.8µm (one end) → chamfering → subcontract Type II anodise clear; lead time 4 weeks, lot 1000/batch',
      },
    ]
