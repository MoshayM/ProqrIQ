...
async function scenario10_BulkCosting() {
  ...
  try {
    const partDefs = [
      {
        part_name:             'Zinc ZP5 Cable Gland M20',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'die_casting',
        material_grade:        'Zinc alloy ZP5, 6.5g/cm³',
        net_weight_g:          55,
        dimensions_json:       { l_mm: 30, w_mm: 30, h_mm: 25, thread_mm: 20 },
        surface_finish:        'Electroless nickel plated 8µm, IP68 rated threads',
        tolerance_class:       'ISO 286 H7, thread ISO 6H',
        manufacturing_process: 'Pressure die casting ZP5 + electroless nickel plating + thread cutting',
      },
      {
        part_name:             'Kapton PI Flex PCB Antenna 80×20mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'flex_pcb',
        material_grade:        'Polyimide Kapton 25µm, 1oz copper, FPC 2-layer',
        net_weight_g:          8,
        dimensions_json:       { l_mm: 80, w_mm: 20, h_mm: 0.2, layers: 2 },
        surface_finish:        'ENIG 1µm Au, coverlay PI 25µm, IPC Class 2',
        tolerance_class:       'Track width ±0.05mm, outer dimension ±0.1mm, bend radius min 1mm',
        manufacturing_process: '2-layer FPC fabrication + ENIG + coverlay lamination + electrical test',
      },
      {
        part_name:             'PETG Transparent Potting Ring Ø45mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'plastic_injection',
        material_grade:        'PETG Transparent 90% light transmission, UL94-HB',
        net_weight_g:          18,
        dimensions_json:       { l_mm: 45, w_mm: 45, h_mm: 12, wall_thickness_mm: 2 },
        surface_finish:        'Optically clear, Ra 0.8µm cavity, draft 2°',
        tolerance_class:       'IT11, inner bore ±0.1mm, height ±0.2mm',
        manufacturing_process: 'Single-cavity injection moulding + automated de-mould + optical inspection',
      },
      {
        part_name:             'SS316L Precision Shaft Ø12mm × L120mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'cnc_machining',
        material_grade:        'Stainless steel SS316L (1.4404), 7.98g/cm³',
        net_weight_g:          145,
        dimensions_json:       { l_mm: 120, diameter_mm: 12, bore_mm: 0, tolerance_it: 6 },
        surface_finish:        'Ground Ra 0.4µm, h6 diameter tolerance ±0.006mm',
        tolerance_class:       'h6 IT6 on shaft diameter, concentricity 0.005mm, straightness 0.01mm/100mm',
        manufacturing_process: 'CNC turning rough + finish → cylindrical grinding h6 → deburring → cleaning',
      },
      {
        part_name:             'PET Membrane Keypad 5-Key 90×60mm',
        part_number:           `BULK-${Math.random().toString(36).slice(2,8).toUpperCase()}`,
        commodity_type:        'membrane_switch',
        material_grade:        'Polyester PET 0.125mm silver-ink printed circuit',
        net_weight_g:          22,
        dimensions_json:       { l_mm: 90, w_mm: 60, h_mm: 1.5, keys: 5, actuation_force_n: 1.5 },
        surface_finish:        'Matte polyester overlay 0.125mm, textured graphic window, backlighting LEDs',
        tolerance_class:       'Key pitch ±0.3mm, overlay dimension ±0.5mm, contact resistance <100mΩ',
        manufacturing_process: 'Screen print silver circuit → punch spacer layer → laminate overlay → die cut → 100% electrical test',
      },
    ]
    ...
    const batch = await post('/bulk-batches', {
      name:        'ProqrIQ E2E Bulk Test — 5 diverse parts 2026-08-09',
      part_ids:    partIds,
      shared_params: {
        supplier_country:        'DE',
        supplier_currency:       'EUR',
