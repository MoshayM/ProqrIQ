...
import {
  X, Bot, Send, User, Loader2, RotateCcw,
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          {/* Drawer — slides in from right, same pattern as notifications */}
          <motion.div
            key="help-drawer"
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            ...
            className="fixed right-0 top-0 bottom-0 w-full sm:w-[400px] bg-white shadow-2xl z-[63] flex flex-col"
          >
            {/* ── Header ── */}
            <div className="flex items-center gap-3 px-5 py-4 border-b border-[#e5e8ef] flex-shrink-0">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                style={{ background: 'linear-gradient(135deg, #1e2d4e 0%, #2d4070 100%)' }}>
                <Bot className="w-5 h-5 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-base font-bold text-[#0f1729] leading-tight">ProqrIQ Assistant</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" />
                  <span className="text-xs text-[#9aa3b2]">AI-powered · always available</span>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            {/* ── Tab strip ── */}
            <div className="flex flex-shrink-0 border-b border-[#e5e8ef]">
              {(['chat', 'guide'] as const).map(t => (
                <button
                  ...
                  className={cn(
                    ...
                    tab === t ? 'text-[#1e2d4e]' : 'text-[#9aa3b2] hover:text-[#4a5568]',
                  )}
                >
                  ...
                  {tab === t && (
                    <motion.div
                      ...
                      className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#1e2d4e] rounded-full"
                      transition={{ type: 'spring', stiffness: 380, damping: 35 }}
                    />
                  )}
                </button>
            ...
            {tab === 'chat' && (
              <>
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 min-h-0 scroll-area">
                  {showWelcome ? (
                    <div className="flex flex-col items-center text-center py-8 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#9aa3b2] mt-1 leading-relaxed max-w-[260px]">
                      ...
                      <div className="w-full flex flex-wrap gap-1.5 justify-center">
                        {QUICK_QUESTIONS.map(q => (
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#f8f9fc] border border-[#e5e8ef] rounded-full text-xs text-[#4a5568] hover:border-navy/30 hover:text-navy transition-colors"
                          >
                            {q}
                          </button>
                        ))}
                ...
                {/* Input area */}
                <div className="flex-shrink-0 px-4 py-4 border-t border-[#e5e8ef]">
                  <div className="flex gap-2 items-end">
                    <textarea
                      ...
                      rows={1}
                      disabled={loading}
                      className="flex-1 resize-none rounded-xl border border-[#e5e8ef] bg-[#f8f9fc] px-3.5 py-2.5 text-sm text-[#0f1729] placeholder-[#c8cdd8] focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy/30 focus:bg-white transition-all disabled:opacity-50"
                      style={{ maxHeight: 100, overflowY: 'auto', fontSize: '16px' }}
                      autoComplete="off"
                      autoCorrect="off"
                      autoCapitalize="off"
                      spellCheck={false}
                    ...
                    <button
                      ...
                      className="p-2.5 rounded-xl bg-navy text-white hover:bg-[#2d4070] disabled:opacity-40 transition-all flex-shrink-0 shadow-sm"
                    >
                      {loading
                  ...
                  <div className="flex items-center justify-between mt-2 px-0.5">
                    {messages.length > 0 ? (
                      <button onClick={reset}
                        className="flex items-center gap-1 text-xs text-[#9aa3b2] hover:text-[#4a5568] transition-colors">
                        <RotateCcw className="w-3 h-3" /> New chat
                      </button>
                    ) : <span />}
                    <p className="text-xs text-[#c8cdd8]">Shift+Enter for new line</p>
                  </div>
                </div>
              </>
            )}
            ...
            {tab === 'guide' && (
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2.5 min-h-0">
                <p className="text-xs font-semibold text-[#9aa3b2] uppercase tracking-wider mb-3">Core features</p>
                {FEATURE_CARDS.map(c => {
                  const Icon = c.icon
                  return (
                    <div key={c.title}
                      className="flex gap-3 p-3.5 rounded-xl bg-white border border-[#e5e8ef] hover:border-[#c8cdd8] transition-colors">
                ...
                <div className="mt-5">
                  <p className="text-xs font-semibold text-[#9aa3b2] uppercase tracking-wider mb-2">Keyboard shortcuts</p>
                        ...
                        <div className="flex items-center gap-1">
                          {keys.map((k, i) => (
                            <React.Fragment key={k}>
                              {i > 0 && <span className="text-[10px] text-[#9aa3b2]">then</span>}
                ...
                <div className="pt-3 pb-2 text-center">
                  ...
                  >
                    Have a question? Ask the AI <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
            )}
